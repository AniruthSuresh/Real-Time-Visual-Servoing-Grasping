...
class Logger:
    ...
    def __init__(self, log_level="debug", name="main"):
        formatter = colorlog.ColoredFormatter(
            "%(log_color)s[%(levelname)s]%(reset)s[%(asctime)s.%(msecs)03d]: "
    ...
    def _to_string(*args, **kwargs):
        str_args = [*map(str, args), *map(lambda item: f"{item[0]} = {item[1]}", kwargs.items())]

        return ", ".join(str_args)

    def debug(self, *args, **kwargs):
        self.debug_msg(self._to_string(*args, **kwargs))

    def info(self, *args, **kwargs):
        self.info_msg(self._to_string(*args, **kwargs))

    def warning(self, *args, **kwargs):
        self.warning_msg(self._to_string(*args, **kwargs))

    def critical(self, *args, **kwargs):
        self.critical_msg(self._to_string(*args, **kwargs))

    def error(self, *args, **kwargs):
        self.error_msg(self._to_string(*args, **kwargs))
    ...
    def set_level(self, log_level):
        ...
        if "debug" in log_level:
            ...
            raise ValueError("Unknown logging " "level: %s" % log_level)
